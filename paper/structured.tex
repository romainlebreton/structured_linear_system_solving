...
$O(\alpha \M(n) \I(2^k))$ operations (even if $\CA$ can be taken less
than $O(\alpha \M(n))$, the multiplications by $\mB_k$ remain a
bottleneck). Since we have to multiply these matrices by $\alpha$
...
$O(\alpha^{\omega-1} \M(n) \I(t))$, but this improvement was not
implemented.
...
our system. Due to the doubling nature of this procedure, the runtime
is proportional to that of the algorithm for solving square systems
used at line 3, plus the cost of rational
reconstruction. The former is $O(\CA \I(t) \log(t) +\alpha \M(n) t)$ bit
operations using divide-and-conquer, and $O(\alpha^2 \M(n) \I(t))$ or
...
We conclude this subsection with a discussion of an important
...
Section~\ref{sec:abstract} is able to exploit this extra structure;
however, in the case of computations over $\Q$, we will now see that
...
e^{(\omega+1)/2} \M(d))$ in this case. To our knowledge, algorithm
$\DACQ$ is the only algorithm that can take into account the extra
structure of algebraic approximants.
...
each level. Although Newton iteration should theoretically be competitive with $\DACQ$ when the number of blocks is small,
Newton iteration is more affected by slow integer multiplication since it computes matrix-matrix products rather than matrix-vector 
products as done by $\DACQ$, which explains why the spikes are sharper.
CRT seems to be somewhat competitive with $\DACQ$ when the number of
blocks is small, but is significantly worse when the number of blocks
grow.
