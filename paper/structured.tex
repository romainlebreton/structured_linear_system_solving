...
\begin{algorithm}[t]
  ...
  $p = \min(m,n), \ i = \lceil p/2 \rceil, \ (\mY^{(0)},\mZ^{(0)}) =
  (\mG,\mH)$\;

  \lIf{$\max(m,n) \leq t$}{\Return $\Iter(\mG,\mH, \alpha)$} 
  $(r_0,\mY^{(r_0,m-r_0)}_1,\mZ^{(r_0,n-r_0)}_1) = 
  \DAC(\mY^{(0,i)}_0, \mZ^{(0,i)}_0,  t)$\;\label{line:dac0}
  ...
  \DAC(\mY',\mZ', t), \ r_2=r_0+r_1$\label{line:dac1}\;
