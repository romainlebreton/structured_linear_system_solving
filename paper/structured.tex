...
\newcommand{\Raise}{\textbf{signal}~error}
...
\acmPrice{15.00}
\acmDOI{http://dx.doi.org/10.1145/3087604.3087659}
...
\begin{abstract}
  ...
  implementations and their practical performance in symbolic
  computation. In this paper, we consider the problem of solving
  Cauchy-like systems, and its application to mosaic Toeplitz systems,
  in two contexts: first in the unit cost model (which is a good model
  for computations over finite fields), then over $\Q$. We introduce
  new variants of previous algorithms and describe an implementation
  of these techniques and its practical behavior. We pay a special
...
Hermite-Pad\'e problem has size
$\sigma \times (n_0 + \cdots + n_{s-1})$, and consists of $s$ lower
triangular Toeplitz blocks. More generally, our goal in this paper is
to compute efficiently elements in the kernel of {\em mosaic Toeplitz}
matrices~\cite{HeAm88}.  An $m \times n$ Toeplitz matrix
...
Toeplitz $\mT=(\mT_{i,j})_{1 \le i \le p,1 \le j \le q}$ of size
$m \times n$ with a $p \times q$ block structure is made of $pq$
blocks $\mT_{i,j}$ being Toeplitz, and so can be described by a
sequence of $pq$ polynomials
$\P=(P_{\mT_{i,j}})_{1 \le i \le p,1 \le j \le q}$.
%
% , together with the sequences $I=(m_1,\dots,m_p)$ and
% $J=(n_1,\dots,n_q)$ giving the row-sizes and column-sizes of the
% blocks.
%
Then, our main problem is as follows.
...
class of Cauchy-like matrices (see definitions below) for which the
...
\mA = \sum_{i=1}^\alpha
\mD_{\vg_i} 
\mC_{\vu,\vv}\,\mD_{\vh_i}= (\mG \mH^t) \odot \mC_{\vu,\vv},\ \ \ 
...
and~$\mH$,  matrix $\mC_{\vu,\vv}$ is known as a {\em Cauchy
  matrix} and $\odot$ is the entry-wise product.
...
%
% The cost for a matrix-vector product by $\mA$ remains
% $O(\alpha \M(\mx))$, but the constant in the big-O is lower:
%
...
%\smallskip
\noindent{\bf \small Reduction from mosaic Toeplitz to
  Cauchy, and regularization.}
...
%
We work here over a field $\K$, and explain how to
solve Problem~\ref{pb:mosaic}  using Pan's reduction to the
Cauchy-like Problem~\ref{pb:cauchy} below. Our goal is to
solve a linear system, but the algorithms do slightly more: they compute
...
%
% (the assumptions on the vectors $\vu,\vv$ are slightly stronger than
...
\begin{pbm}\label{pb:cauchy}
  ...
  If $\mA$ does not have generic rank profile, signal an error;
...
Problem~\ref{pb:cauchy}, iterative ones, of cost $\Theta(mn)$ (for
fixed $\alpha$), and divide-and-conquer algorithms, of quasi-linear
cost in $m+n$. We stress that having a fast quadratic-time algorithm
is actually crucial in practice: as is the case for the Half-GCD, fast
linear algebra algorithms, etc, the divide-and-conquer algorithm will
fall back on the iterative one for input sizes under a certain
threshold, and the performance of the latter will be an important
factor in the overall runtime.

Iterative algorithms that solve a size $n$ Toeplitz system in time
$O(n^2)$ have been known since~\cite{Levinson47,Durbin60,Trench64}; extensions to structured
matrices were later given, as e.g. in~\cite{KaGoOl95}. After a few preliminary results, we give in~\ref{ssec:quad} an
...
In~\ref{ssec:MBA-C}, we review divide-and-conquer
...
$ \mA=\left [\begin{smallmatrix}
  ...
  \end{smallmatrix} \right]$
with $\mA^{(i)}_{0,0} \in \K^{i \times i}$, and
$$
\mS^{(i)} = 
\begin{bmatrix} 
...
% To understand where these matrices come from, we introduce the matrix
...
%
Let $i,j$ be such that $0 \le i+j \le \mn$, and
...
generalizes the previous formulae for $(\mY^{(i)},\mZ^{(i)})$ :
\begin{align}
...
%% \smallskip\noindent{\bf \small Termination and correctness.}
% For the correctness of the algorithm, note that $\mA$ has generic rank
...
Following this discussion, we describe in Algorithm~\ref{algo:iter} a
quadratic iterative algorithm for Problem~\ref{pb:cauchy}, using a
step size $\beta \in \{1,\dots,\alpha\}$ given as a parameter.
Computing the rank profile (line~\ref{line:rankprof}) costs
$O(\beta^\omega)$.  Using block matrix multiplication in
Eq.~\eqref{eq:recA}, we recover $\mS^{(i,j)}_{0,1}$ from its $\nabla_{\vu^{(i,j)}_0,
  \vv^{(i,j)}_1}$-generators $(\mY^{(i,j)}_0,\mZ^{(i,j)}_1)$,
and similarly for
$\mS^{(i,j)}_{1,0}$, in time
$O(\beta^{\omega-2} \alpha (m+n))$, since $j \le \beta \le \alpha$
(line~\ref{line:rowcol}).
  To compute $\mY^{(i+j)}$ at
...
$(\mY^{(\bar{r})},\mZ^{(\bar{r})})$ using~\eqref{eq:recA} in time
 $O(\alpha^{\omega-2} mn)$, by block matrix multiplication.

One iteration of the while loop costs $O(\beta^{\omega-2} \alpha
(m+n))$; we iterate $O(\mn/\beta)$ times, for a total of
$O(\beta^{\omega-3} \alpha \mn(m+n))$, which is $O(\beta^{\omega-3}
\alpha mn)$. This dominates the cost of line~\ref{line:recoverwhole},
so the whole runtime is $O(\beta^{\omega-3} \alpha mn)$. The
algorithm of~\cite{Mouilleron08} uses $\beta=1$, for which the cost is
$O(\alpha mn)$; choosing $\beta=\alpha$, we benefit from fast matrix
multiplication, as the cost drops to $O(\alpha^{\omega-2} mn)$. This
theoretical improvement has also practical implications : $\omega < 3$
is achieved in practice~\cite{DGP08,BoDu16}, and linear algebra
implementations are highly optimized and gain a significant constant
factor even for $\omega = 3$.
...
\setstretch{0.1}
  ...
  \While{$i \neq \bar{r}$}{
    ...
    \lIf{$\mS^{(i,j)}_{0,0}$ has non generic rank profile}{\Raise}
...
our framework, with the minor difference that we do not assume $\mA$
...
% so we assume that we are not in that case; then, the correctness proof
% is similar to that in Section~\ref{ssec:quad}. 
%
Step~\ref{line:recoverfullgen0},~\ref{line:recoverfullgen1} are similar: using Eq.~\eqref{eq:Si+j} and \eqref{eq:Yi+j}, we
recover generators $(\mY^{(i+j)},\mZ^{(i+j)})$ of
$\mS^{(i+j)}$ from 
...
formulae of~\eqref{eq:Yi+j} do $O(1)$ multiplications of
...
\noindent
Our goals are to assess if matrix
multiplication can bring practical improvements that reflect the
theoretical ones, find  crossover points between
...
implementations of the various techniques seen so far.

NTL already offers efficient FFT-based polynomial
...
Our solution of Problem~\ref{pb:mosaic} is by a direct reduction to
the Cauchy-like case. Putting the problem into Cauchy form  accounts for a small
...
sizes such as $n \ge 5000$ and small primes $p \simeq 2^{16}$, we
...
We now address Problem~\ref{pb:mosaic} for $\K=\Q$. Several {\em
  modular algorithms} are available to solve dense linear systems over
$\Q$. A first option relies on the Chinese Remainder Theorem, solving
the system modulo several primes $p_1,p_2,\dots$ before reconstructing
the solution, making sure to ensure consistency of the modular
solutions when ${\rm ker}(\mT)$ has dimension more than $1$. Other
approaches such as Newton iteration or divide-and-conquer algorithms
use one prime $p$, and lift the solution modulo powers of $p$ (Newton
iteration for structured matrices goes back to~\cite{Pan92}; see
details in~\cite[Chapter~7]{Pan01}; the divide-and-conquer approach is
discussed in~\cite{Lebreton12}). Our goal here is to first briefly
present these techniques and analyze their complexity; we will then
discuss their practical performance. We will highlight in particular
the case of algebraic approximants, for which we are able to obtain
...
We are given  a prime power $p^t$, where $t$ is a
power of 2, an $n \times n$ matrix $\mA$ and a vector $\vb$, both with
entries modulo $p^t$, such that $\mA$ is invertible modulo
$p^t$, with inverse $\mB$; we discuss algorithms that solve the
equation $\mA \vx = \vb$. To simplify the cost analysis, we assume here
that $p=O(1)$.
We take $\CA$ such that, for any $t$, we can compute $\mA \vx \bmod
p^t$ in $O(\CA \I(t))$ bit operations, for a vector $\vx$ with
...
length $\alpha$, so we can take $\CA \in O(\alpha \M(n))$. We
...
% \vspace{-7px}
\noindent Assume that we have know generators of length $\alpha$ for $\mB$.  Thus,
at the leaves of the recursion, each product $\mB \vb$ mod $p$ can be
computed using $O(\alpha \M(n))$ bit operations; with our assumption
on $\I$, the total runtime is $O(\CA \I(t) \log(t) +\alpha \M(n)
t)$ bit operations. With our upper bounds on $\CA$, this simplifies
 as $O(\alpha \M(n) \I(t) \log(t))$.
...
% \vspace{-4mm}
\begin{align*}
\mX_{k+1} &= -(2\mB_k - \mB_k \mA \mB_k)\phantom{^t\,} \mG \bmod p^{2^{k+1}}\\[-1mm]
...
$O(\alpha \M(n) \I(2^k))$ operations. Since we  multiply these matrices by $\alpha$
...
In order to solve instances of Problem~\ref{pb:mosaic}, that is, find
nullspace elements for a mosaic Toeplitz matrix $\mT$, we rely on the
transformation to the Cauchy structure / regularization technique of
Section~\ref{sec:basics}. Over $\Q$, this approach has a certain shortcoming: the output is a
vector
%
...
issue: in this case, all solutions are of the form
...
$N$ and normalize it by setting one of its entries to $1$. 
In general, we reduce the nullspace dimension by adding a new block of
equations; when ${\rm ker}(\mT)$ has moderate dimension, this barely
affects the overall runtime (for particular applications to algebraic
approximants, another solution is described below).

For simplicity, we give here a solution that may
run forever in unlucky cases. Choose a prime $p$ and compute
$\nabla_{\vu,\vv}$-generators for $\mA = \mV_\vu \mT \mW_{\vv}$ (mod
$p$), for $\vu,\vv$ as in Remark~\ref{rmk:factor3}. Call the
algorithm of the previous section, to determine the rank of
$\mA$; if $\mA$ does not have generic rank profile, choose another
$\vu,\vv$. After an expected $O(1)$ attempts, we obtain the rank of
$\mA$  over $\mathbb{F}_p$, and thus the dimension $s$ of
...
divide-and-conquer methods, and $O(\alpha^2 \M(n) \I(t))$ or
