...
then the above lemma shows that for $i=\rank(\mA)$, $(\mY^{(r)}_0,\mZ^{(r)}_0)$
...
\begin{algorithm}[t]
  ...
  \While{$i \neq r_{\max}$}{
    ...
    \lIf{$\rank( \mS^{(i,j)}_{1,1})\!<\!j$}{
      $j=\rank( \mS^{(i,j)}_{1,1}), r_{\max}=i+j$}
  ...
  Recover $\mS^{(r_{\max})}$ from its generators $(\mY^{(r_{\max})},\mZ^{(r_{\max})})$\;\label{line:recoverwhole}
  Read in $\mS^{(r_{\max})}$ the Schur complement of $\mA^{(r_{\max})}_{0,0}$ in $\mA$\;
  \lIf{Schur complement is non zero}{\Return FAIL}
  \lElse {\Return $(r_{\max},\mY^{(r_{\max})}_0,\mZ^{(r_{\max})}_0)$}
\end{algorithm}

We describe in Algorithm~\ref{algo:quadratic} an quadratic iterative
...
$r_{\max} = \max\{ r \ |\ \forall j \leq r, \det(\mA^{(j)}_{0,0}) \neq
0 \}$.
Either $\rank(\mS^{(i,j)}_{1,1})$ is always $j$, then $i$ will
eventually reach $r_{\max}=p$. Or $\rank(\mS^{(i,j)}_{1,1})<j$ at some
point and after setting $r_{\max}=i+\rank( \mS^{(i,j)}_{1,1})$, we
have $\det(\mA^{(\ell)}_{0,0}) \neq 0$ for all $\ell \leq r_{\max}$
and $\det(\mA^{(r_{\max}+1)}_{0,0}) = 0$, and the while loop exits
right after.

After the while loop, it remains to test if $\rank(\mA) = r_{\max}$
which is done by checking that the Schur complement of
$\mA^{(r_{\max})}_{0,0}$ in $\mA$ is zero.
...
matrix $\mS^{(r_{\max})}$ from its
$\nabla_{\ve^{(r_{\max})},\vf^{(r_{\max})}}$-generators
$(\mY^{(r_{\max})},\mZ^{(r_{\max})})$ is done by means
