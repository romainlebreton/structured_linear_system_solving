...
following graph shows the ratio Time(our algorithm) / Time(previous
algorithm), for $\alpha=30$. We see that for small primes, for which
matrix multiplication is very efficient, our algorithm brings a
...
\times \alpha$ dense matrix. We first compare the direct approach of
...
typical FFT starcaise behavior. This in turn implies that as $n$
grows, the crossover value for $\alpha$ fluctuates, roughly between
$30$ and $55$. The following graph shows the time ratio Time(algorithm
of~\cite{BoJeMoSc16}) / Time(direct algorithm), for $\alpha=60$; at
...
and switch to the quadratic algorithm. For $\alpha \le 10$, taking
$n_0\simeq 200$ is a good choice; for $\alpha \ge 20$, we take $n_0 =
500$ (and we interpolate linearly between the two). The crossover
points are essentially the same for small (less than $2^{23}$) or
large word size primes. These values being set, we can then show
runtimes for solving Problem~\ref{pb:cauchy} for $\alpha=5$ and
$\alpha=50$, and increasing values of $n$; we also display the time
taken by a dense matrix inversion, as a matter of reference (this does
not depend on the choice of $\alpha$).
