...
We take $\CA$ such that, for any $t$, we can compute
$\mA \vx \bmod p^t$ in $O(\CA \I(t \log(p)))$ bit operations, given a
vector $\vx$ with entries defined modulo $p^t$. Below, we will assume
that $\mA$ is Cauchy-like as in Remark~\ref{rmk:factor3}, and given by
generators of length $\alpha$, so that we can take
$\CA \in O(\alpha \M(n))$. We will see below that in the case of
algebraic approximants, better estimates are available.
