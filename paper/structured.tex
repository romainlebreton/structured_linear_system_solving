...
$O(\beta^{\omega-2} \alpha (m+n))$; we iterate $O(\mn/\beta)$ times,
for a total cost of $O(\beta^{\omega-3} \alpha \mn(m+n))$, which is
...
cost drops to $O(\alpha^{\omega-2} mn)$. Note that this theoretical
improvement by a factor $\alpha^{3-\omega}$ has also practical
implications : first $\omega < 3$ is achieved also in
practice~\cite{DGP08,BoDu16} and second linear algebra implementations
are highly optimized and gain a significant constant factor even for
$\omega = 3$.
...
the Cauchy-like case. Putting the problem into Cauchy form by means of
the formulae of Eq.~(\ref{eq:gentoetogencau}) accounts for a small
...
computed using $O(\alpha \M(n))$ bit operations; applying our assumption
