...
\begin{algorithm}[t]
  ...
  \While{$i \neq \bar{r}$}{
    ...
    Compute the rank $\rho$ and the rank profile of $\mS^{(i,j)}_{0,0}$\; \label{line:rankprof}
    \If{$\mS^{(i,j)}_{0,0}$ does not have generic rank profile}{
      \Raise\;
    }
    \lIf{$\rho<j$}{$j=\rho,\ \bar{r}=i+\rho$}
    Recover the row $\mS^{(i,j)}_{0,\ast}$ and column $\mS^{(i,j)}_{\ast,0}$\; \label{line:rowcol}
  ...
  \lElse {\Return $(\bar{r},\mY^{(\bar{r},n-\bar{r})}_1,\mZ^{(\bar{r},m-\bar{r})}_1)$}
\end{algorithm}
...
$(\mY^{(i)},\mZ^{(i)})$ only involves the block matrices of
$\mS^{(i)}$ that belong to the row or the column of
$\mS^{(i,j)}_{0,0}$. So Algorithm~\ref{algo:iter},
line~\ref{line:rowcol} recovers the row $\mS^{(i,j)}_{0,\ast}$ from
its $\nabla_{\vu_{i+1:i+j}, \vv^{(i)}}$-generators
$(\mY^{(i,j)}_0,\mZ^{(i)})$ and the column $\mS^{(i,j)}_{\ast,0}$ from
...
\begin{algorithm}[t]
  ...
  \Output{$r=\rank(\mA)$, generators $(\mY^{(r)}_0,\mZ^{(r)}_0)$ of
    $\mA^{(r)}_{0,0}{}^{-1}$}

  $p = \min(m,n), p' = \max(m,n), (\mY^{(0)},\mZ^{(0)}) = (\mG,\mH)$\; 

  \lIf{$p' \leq t$}{\Return $\Iter(\mG,\mH, \alpha)$} 
  $i = \lceil p/2 \rceil$\;
  $(r_0,\mY^{(r_0)}_0,\mZ^{(r_0)}_0) = \DAC(\mG^{(i)}_0, \mH^{(i)}_0,  t)$\;\label{line:dac0}

  Compute $(\mY^{(r_0)},\mZ^{(r_0)})$ from
  $(\mY^{(r_0)}_0,\mZ^{(r_0)}_0)$, $(\mY^{(0)},\mZ^{(0)})$
