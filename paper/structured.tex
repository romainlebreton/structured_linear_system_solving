...
We now point out a simpler property, involving only  $\mA' = \mV_\vu\, \mA\,
...
generators of $\mA_{r}^{-1}$, where $\mA_r$ is a maximal leading minor
...
Problem~\ref{pb:cauchy}, iterative ones, of cost that grows like $mn$
(for fixed $\alpha$), and algorithms using divide-and-conquer
techniques, of quasi-linear cost in $m+n$. We stress that having a
...
matrices of size $n$, this results in an algorithm of cost $O(\alpha^2
\M(n)\log(n)^2)$. As pointed out in~\cite[Theorem~5.3.1]{Pan01} (see
also~\cite{ChPa00}), the cost can be reduced to $O(\alpha^2
\M(n)\log(n))$ by choosing vectors $\vu$ and $\vv$ in geometric
progression. In our case, we will use vectors $\vu$, $\vv$ that are 
in geometric progression with the same ratio, in order to apply the remark
...
\begin{algorithm}[t]
  ...
  \Input{Generators $(\mG,\mH)$ of $\mA$, threshold $n_0$}
  \Output{$r=\rank(\mA)$, generators of $\mA^{(r)}_{0,0}{}^{-1}$}

  $\mn = \min(m,n), \ i = \lceil \mn/2 \rceil, \ (\mY^{(0)},\mZ^{(0)}) =
  (\mG,\mH)$\;

  \lIf{$\max(m,n) \leq n_0$}{\Return $\Iter(\mG,\mH, \alpha)$} 
  $(r_0,\mY^{(r_0,m-r_0)}_1,\mZ^{(r_0,n-r_0)}_1) = 
  \DAC(\mY^{(0,i)}_0, \mZ^{(0,i)}_0,  n_0)$\;\label{line:dac0}

  Recover Schur complement
  gen. $(\mY^{(r_0,m-r_0)}_0,\mZ^{(r_0,n-r_0)}_0)$ $=(\mY',\mZ')$ from
  ...
  \DAC(\mY',\mZ', n_0), \ r_2=r_0+r_1$\label{line:dac1}\;
...
matrices (for which we have genertors of length $\alpha$) 
by $\alpha$ vectors.

In general, this multiplication $O(\alpha^2 \M(\mx) \log(\mx))$
operations, with $\mx=\max(m,n)$. If if $\vu$ and $\vv$ are chosen as
...
we saw that can save a  further constant factor.  Finally, for large values of
$\alpha$, we can apply an algorithm given in~\cite{BoJeMoSc16}, which
reduces the cost to $O(\alpha^{\omega-1} \M(\mx))$.
...
takes time $O(\alpha^{\omega-1} \mx)$ as well).

Taking all recursive steps into account, the total cost of $\DAC$ is
...
values $n_0(\alpha)$ to end the recursion in the divide-and-conquer algorithm
and switch to the iterative algorithm. We expect 
$n_0(\alpha)$ to grow with $\alpha$, since for a given $\alpha$, it behaves
...
its inverse). We discuss algorithms that solve the equation $\mA \vx =
\vb$ modulo $p^{t}$, where $t$ is a power of 2, and we estimate their
complexity when $\mA$ is a structured matrix. 
...
assume that $\mA$ is Cauchy-like as in~\eqref{eq:recA}, with vectors
$\vu,\vv$ geometric progressions have the same ratio, and given by
generators of length $\alpha$, so that we can take $C_\mA \in O(\alpha
\M(n))$. We saw in the introduction that in some cases (for the
calculation of algebraic approximants, for instance), better estimates
are available.
...
\noindent{\bf 1.} Compute $\vx_0 := $ DAC($\mA$, $\vb$, $p$, $t/2$)

\noindent{\bf 2.} Compute $\vr_0 := \mA  \vx_0 - \vb$ mod $p^t$ and $\vr_1 := r_0/ p^{t/2}$ 

\noindent{\bf 3.} Compute $\vx_1 := $ DAC($\mA$, $\vr_1$, $p$, $t/2$)
