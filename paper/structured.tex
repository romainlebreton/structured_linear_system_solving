...
$\nabla_{\vu,\vv}$-generators of a matrix $\mA$, with $\vu=(u_1,\dots,u_m)$ and
...
$
If ${\mA^{(i)}_{0,0}}$ is invertible, set
$$
\mS^{(i)} = 
\begin{bmatrix} 
...
Problem~\ref{pb:cauchy}. The algorithm essentially
...
all recursive calls), we saw in Remark~\ref{rmk:factor3} that can save
a further constant factor.  For large values of $\alpha$, we
can apply an algorithm given in~\cite{BoJeMoSc16}, which reduces the
...
polynomial arithmetic; matrix multiplication over small fields $\Z/p\Z$, for $p
...
In what follows, we denote by $\mathscr{I}:\mathbb{N}
\to \mathbb{R}$ a function such that integers of bit size at most $d$
can be multiplied in $\mathscr{I}(d)$ bit operations; using FFT
techniques, we can take $\mathscr{I}(d)=O(d \log(d) \log\log(d))$ or
$\mathscr{I}(d)=d \log(d) 2^{O(\log^*(d))}$~\cite{ScSt71,Furer07}. As
in~\cite{GaGe13}, we assume that $d\mapsto \mathscr{I}(d)/d$ is
non-decreasing.  
...
$p^t$, with inverse $\mB$. We discuss algorithms that
solve the equation $\mA \vx = \vb$, and we estimate
their complexity when $\mA$ is a structured matrix.

We take $\CA$ such that, for any $t$, given a vector $\vx$ with
entries defined modulo $p^t$, we can compute $\mA \vx \bmod p^t$ in
$O(\CA \I(t \log(p)))$ bit operations. Below, we will assume that
$\mA$ is Cauchy-like as Remark~\ref{rmk:factor3}, and given by
generators of length $\alpha$, so that we can take $\CA \in O(\alpha
\M(n))$. We will see below that in the case of algebraic approximants,
...
precision $t$ (the runtime of Dixon's algorithm is linear in $t$ only
when the entries of $\mA$ are assumed to be $O(1)$; if $\mA$ has
arbitrary entries, Dixon's algorithm takes quadratic time in $t$ even
using fast integer arithmetic).  We start with the divide-and-conquer
approach. A version of it is in~\cite{BeLe12} for dense matrices; the
PhD thesis~\cite{Lebreton12} describes this algorithm for
...
\setstretch{0.1}
  ...
  \Input{$\mA$, $\mB$ $\vb$, $p$, $t$ as above}
  \Output{a solution of  $\mA \vx = \vb$ mod $p^t$}

\lIf{$t = 1$}{\Return $\mB \, \vb$ mod $p$}

Compute $\vx_0 = $\DACQ($\mA$, $\vb$, $p$, $t/2$)\;

Compute $\vr_0 = \left( \mA \vx_0 - \vb\right) \bmod p^t$ and
$\vr_1 = r_0/ p^{t/2}$\;

Compute $\vx_1 = $\DACQ($\mA$, $\vr_1$, $p$, $t/2$)\;

\Return $\vx_0 - p^{t/2}  \vx_1$ mod $p^t$
\end{algorithm}

Assume that we have obtained generators of length $\alpha$ for
...
can be computed using $O(\alpha \M(n) \I(\log(p)))$ bit
...
\end{align*}
This time, we are multiplying $\mA$ and $\mB_k$ (and their
...
solve the system modulo $t=2^k$ primes of the same magnitude. 
...
is now $O(\alpha^{\omega-1} \M(n) \log(n) t)$, by the results 
of Section~\ref{sec:abstract}.
...
dimension, but it would be tantamount to computing and lifting a whole
nullspace basis. Instead, we chose to reduce the nullspace dimension
by adding a new of equations, as discussed below; when ${\rm
  ker}(\mT)$ has moderate dimension, this barely affects the overall
runtime (for particular applications to algebraic approximants,
...
submatrix of $\mA$, and $\vb_d$ be a vector of the first $d$ entries
of the last column $\vb$ of $\mA$. We assume that $\mA$ has generic
rank profile, so that $\mA_{d}$ is invertible.  We compute $\vx =
\mA_d^{-1} \vb_d$ (mod $p$) and $\vy = \mW_\vv [x_1, \cdots, x_d,
  -1]^t$ (mod $p$). Finally, we normalize $\vy$ by dividing it by its
first non-zero entry, and we set $t=1$.
While we either cannot apply rational reconstruction to the entries of
$y$, or after applying rational reconstruction to $\vy$ we have $\mM
...
Assuming the lifting algorithm stops, we have obtained a solution to
our system. Due to the doubling nature of this procedure, the runtime is 
proportional to that of the algorithm for solving square systems used 
at line 3, that is, $O(\CA \I(t) \log(t) +\alpha \M(n) t)$
or $O(\alpha^2 \M(n) \I(t))$ bit operations, if  $p=O(1)$.

\subsection{Experimental Results}
The timings in this section are measured on the same machine
as section 3.4. The main goal of our experiment is to compare
the runtimes for each algorithm and assess which one is the most
efficient in practice. With these experiments, there are two main
parameters: the number of blocks, and the size of each block. Although
we also have control over the bit sizes of the input and output, our
...
columns, and on the right, we have $n$ blocks, each with 
$n$ columns, where we let $n$ grow. Every matrix generated has exactly
$n-1$ rows. We first examine the results for the general case.
