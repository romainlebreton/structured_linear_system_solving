...
\begin{align}
\mY^{(i)}_0&= \mA^{(i)}_{1,0} \mY^{(i)}_1 + \mG^{(i)}_0,\label{eq:defYi}
&
\mY^{(i)}_1&= -{\mA^{(i)}_{0,0}}{}^{-1} \mG^{(i)}_1,\  
\\
\mZ^{(i)}_0&=-{\mA^{(i)}_{0,1}}{}^t \mZ^{(i)}_1 + \mH^{(i)}_0\label{eq:defZi}.
&
\mZ^{(i)}_1&= {\mA^{(i)}_{0,0}}^{-t} \mH^{(i)}_1,\  
\end{align}
...
\begin{algorithm}[t]
  ...
  \While{$i \neq \bar{r}$}{
    ...
    Compute the rank $\rho$ and the rank profile of $\mS^{(i,j)}_{1,1}$\; \label{line:rankprof}
  ...
  \lIf{Schur complement is non zero}{\Raise}
