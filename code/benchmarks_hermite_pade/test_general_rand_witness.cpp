...
void run_DAC(const Vec<ZZX> &f, const Vec<long> &type, long prec){
  cout << "Running DAC using a witness" << endl;

  Vec<long> witness;
  zz_p::FFTInit(1);
  
  hermite_pade_general hp2(f, type, prec, 1);
  Vec<zz_pX> sol_p2;
  hp2.random_solution_mod_p(sol_p2);
  for (long i = 0; i < type.length(); i++)
    for (long j = 0; j < type[i]+1; j++)
      witness.append(conv<long>(coeff(sol_p2[i], j)/coeff(sol_p2[0], deg(sol_p2[0]))));

  double t = GetTime();
  hermite_pade_general hp(f, type, prec, witness, 0);
  hp.switch_mode(0);
  Vec<ZZX> sol;
